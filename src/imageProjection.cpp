...
public:
    ImageProjection() : deskewFlag(0)
    {
        ...
        pubExtractedCloud = nh.advertise<sensor_msgs::PointCloud2>("e_adaplio/deskew/cloud_deskewed", 1);
        pubLaserCloudInfo = nh.advertise<e_adaplio::cloud_info>("e_adaplio/deskew/cloud_info", 1);
    ...
    void findPosition(double relTime, float *posXCur, float *posYCur, float *posZCur)
    {
        ...
        *posZCur = 0;
        ...
        // 下面如果是无人机、汽车等雷达的载体，最好将一下代码注释去掉（因为速度快，就不能近似不动）。
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            if (KITTI_OR_SOUREC)
            {
                ...
                else
                {
                    ...
                    if (N_SCAN == 64)
                    {
                        // 将垂直角从 -24.8 到 +2 映射到 0 到 63 的索引范围
                        rowIdn = int((verticalAngle + 24.8) / (26.8 / (N_SCAN - 1)) + 0.5);
                        if (rowIdn < 0 || rowIdn >= N_SCAN)
                        {
                            continue; // 超出合法范围，跳过该点
                        }
                        else if (rowIdn % downsampleRate != 0)
                        {
                            continue; // 不符合降采样要求，跳过该点
                        }
                    }
                }
